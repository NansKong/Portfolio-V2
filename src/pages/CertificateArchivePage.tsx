import { archiveCertificates } from "../archiveData";
        ...
        <table className="archive-table" style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.875rem" }}>
          <thead>
            <tr style={{ borderBottom: "1px solid rgba(148,163,184,0.1)" }}>
              {["Year", "Certificate", "Issued By", "Link"].map((h) => (
                <th key={h} style={{
                  textAlign: "left",
                  paddingBottom: "16px",
                  paddingRight: "24px",
          ...
          <tbody>
            {archiveCertificates.map((c, i) => (
              <ArchiveCertificateRow key={i} certificate={c} />
            ))}
          </tbody>
        </table>
      </div>
...
function ArchiveCertificateRow({ certificate }: { certificate: (typeof archiveCertificates)[0] }) {
  return (
